...
static void reset_collision_cache(UBYTE direction);
			...
			case DIR_UP:
				...
				switch(tile_id){					
					...
					case 157://1up block					
						...
						hit_metatile_x = tile_x;
						hit_metatile_y = tile_y;
						hit_metatile_source = 0;
						plat_callback_execute(HIT_BLOCK);
						break;
				}
				break;
		}
...
#endif
			on_player_metatile_collision(tile_hit_x, tile_hit_y, moving_right ? DIR_RIGHT: DIR_LEFT); 
        } else {
			reset_collision_cache(moving_right ? DIR_RIGHT: DIR_LEFT);
		}
