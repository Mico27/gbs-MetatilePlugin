...
void platform_init(void) BANKED {
    ...
    if (plat_camera_block & 1){
        edge_left = ((camera_x >> 4) - (SCREENWIDTH >> 1)) << 4;
    }
    else{
        edge_left = 0;
    }
    ...
    if (plat_camera_block & 2){
        edge_right = ((camera_x >> 4) + (SCREENWIDTH >> 1) - PLAYER.bounds.right) << 4;
    }
    else{
        edge_right = image_width << 4;
    }
...
void fall_state(void) BANKED {
    ...
    gotoXCol:
    {
        ...
        if (new_x > PLAYER.pos.x) {
            tile_x = col_tile_x = ((new_x >> 4) + PLAYER.bounds.right) >> 3;
			if (warp_level && col_tile_x >= image_tile_width){
				if (deltaX < 0){
					col_tile_x = image_tile_width - 1;
				} else {
					col_tile_x = 0;
				}				
			}            
            while (tile_start < tile_end) {
                ...
                col = tile_at(col_tile_x, tile_start);
                if (col & COLLISION_LEFT) {				
					new_x = (((tile_x << 3) - PLAYER.bounds.right) << 4) - 1;
					pl_vel_x = 0;
					col = 1;
					last_wall = 1;	
					on_player_metatile_collision(col_tile_x, tile_start, DIR_RIGHT); 					
					break;
                } else {
            ...
            tile_x = col_tile_x = ((new_x >> 4) + PLAYER.bounds.left) >> 3;
			if (warp_level && col_tile_x >= image_tile_width){
				col_tile_x = image_tile_width - 1;
			} 
            while (tile_start < tile_end) {
                ...
                if (col & COLLISION_RIGHT) {					
                    ...
                    col = -1;
                    last_wall = -1;		
					on_player_metatile_collision(col_tile_x, tile_start, DIR_LEFT); 					
                    break;
                } else {
    ...
    //Y-COLLISION
    {
        ...
        if (deltaY > 0) {
            ...
            if (nocollide == 0){
                ...
                while (tile_start < tile_end) {
					if (warp_level && tile_start >= image_tile_width){
						tile_start -= image_tile_width;
						tile_end -= image_tile_width;						
					}  
        ...
        if (new_x > PLAYER.pos.x) {
            ...
            while (tile_start < tile_end) {
                ...
                if (col & COLLISION_LEFT) {				
					...
					last_wall = 1;
					if (grounded){					
						switch(sram_map_data[METATILE_MAP_OFFSET(col_tile_x, tile_start)]){
                ...
                while (tile_start < tile_end) {
                    ...
                    if (tile_at(tile_start, tile_y) & COLLISION_TOP) {	
						...
						if (is_leftmost == 1 && INPUT_DOWN){//check only mario's left leg for left pipe part
							UBYTE tile_id = sram_map_data[METATILE_MAP_OFFSET(tile_start, tile_y)];
