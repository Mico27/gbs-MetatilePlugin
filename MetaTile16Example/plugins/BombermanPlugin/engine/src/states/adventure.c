...
//Bomberman
UBYTE bomb_count_upgrade;
UBYTE bomb_range_upgrade;
UBYTE bomb_count;
UBYTE bomb_state[MAX_BOMBS];
UBYTE bomb_posx[MAX_BOMBS];
UBYTE bomb_posy[MAX_BOMBS];
UBYTE bomb_timer[MAX_BOMBS];
UBYTE bomb_fire_top[MAX_BOMBS];
UBYTE bomb_fire_right[MAX_BOMBS];
UBYTE bomb_fire_bottom[MAX_BOMBS];
UBYTE bomb_fire_left[MAX_BOMBS];

UBYTE level_bomb_upgrade_posx[MAX_LEVEL_BOMBS_UPGRADES];
UBYTE level_bomb_upgrade_posy[MAX_LEVEL_BOMBS_UPGRADES];
UBYTE level_bomb_upgrade_type[MAX_LEVEL_BOMBS_UPGRADES];

const UBYTE explosion_anim_offset_lookup[] = { 1, 2, 3, 3, 2, 1, 0, 0 };
...
static void update_bomb_blast_tile(UBYTE x, UBYTE y, UBYTE end_tile_id){	
	UBYTE tile_id = sram_map_data[METATILE_MAP_OFFSET(x, y)];
	switch(tile_id){
		case 1:
		case 2:
		case 3:
		...
		case 17:
		case 18:
			for (UBYTE i = 0; i < MAX_BOMBS; i++){
				if (bomb_state[i] == 2 && bomb_posx[i] == x && bomb_posy[i] == y){
					bomb_state[i] = 3;
					break;
				}				
			}
		break;
		default:
			if (!sram_collision_data[TILE_MAP_OFFSET(tile_id, x, y)]){
				replace_meta_tile(x, y, end_tile_id, TRUE);
			}
		break;
	}
}

static void reset_bomb_blast_tile(UBYTE x, UBYTE y){
	UBYTE tile_id = ReadBankedUBYTE(image_ptr + (UWORD)(((y >> 1) * (image_tile_width >> 1)) + (x >> 1)), image_bank);
	if (tile_id != 5){
...
static void update_bomb_blast(UBYTE bomb_idx) {
	if (bomb_timer[bomb_idx]){
		UBYTE tile_anim_offset = explosion_anim_offset_lookup[(bomb_timer[bomb_idx] & 7)] << 4;		
		replace_meta_tile(bomb_posx[bomb_idx], bomb_posy[bomb_idx], 48 + tile_anim_offset, TRUE);		
		for (UBYTE y = bomb_posy[bomb_idx] + 2; y < bomb_fire_bottom[bomb_idx]; y+=2){
			replace_meta_tile(bomb_posx[bomb_idx], y, 50 + tile_anim_offset, TRUE);
		}
		for (UBYTE y = bomb_posy[bomb_idx] - 2; y > bomb_fire_top[bomb_idx]; y-=2){
			replace_meta_tile(bomb_posx[bomb_idx], y, 50 + tile_anim_offset, TRUE);
		}
		for (UBYTE x = bomb_posx[bomb_idx] + 2; x < bomb_fire_right[bomb_idx]; x+=2){
			replace_meta_tile(x, bomb_posy[bomb_idx], 53 + tile_anim_offset, TRUE);
		}
		for (UBYTE x = bomb_posx[bomb_idx] - 2; x > bomb_fire_left[bomb_idx]; x-=2){
			replace_meta_tile(x, bomb_posy[bomb_idx], 53 + tile_anim_offset, TRUE);
		}		
		update_bomb_blast_tile(bomb_posx[bomb_idx], bomb_fire_top[bomb_idx], 49 + tile_anim_offset);
		update_bomb_blast_tile(bomb_posx[bomb_idx], bomb_fire_bottom[bomb_idx], 51 + tile_anim_offset);
		update_bomb_blast_tile(bomb_fire_left[bomb_idx], bomb_posy[bomb_idx], 52 + tile_anim_offset);
		update_bomb_blast_tile(bomb_fire_right[bomb_idx], bomb_posy[bomb_idx], 54 + tile_anim_offset);
				
	} else {		
		reset_bomb_blast_tile(bomb_posx[bomb_idx], bomb_posy[bomb_idx]);		
		for (UBYTE y = bomb_posy[bomb_idx] + 2; y < bomb_fire_bottom[bomb_idx]; y+=2){
			reset_bomb_blast_tile(bomb_posx[bomb_idx], y);
		}
		for (UBYTE y = bomb_posy[bomb_idx] - 2; y > bomb_fire_top[bomb_idx]; y-=2){
			reset_bomb_blast_tile(bomb_posx[bomb_idx], y);
		}
		for (UBYTE x = bomb_posx[bomb_idx] + 2; x < bomb_fire_right[bomb_idx]; x+=2){
			reset_bomb_blast_tile(x, bomb_posy[bomb_idx]);
		}		
		for (UBYTE x = bomb_posx[bomb_idx] - 2; x > bomb_fire_left[bomb_idx]; x-=2){
			reset_bomb_blast_tile(x, bomb_posy[bomb_idx]);
		}
		reset_bomb_blast_tile(bomb_posx[bomb_idx], bomb_fire_top[bomb_idx]);
		reset_bomb_blast_tile(bomb_posx[bomb_idx], bomb_fire_bottom[bomb_idx]);
		reset_bomb_blast_tile(bomb_fire_left[bomb_idx], bomb_posy[bomb_idx]);
		reset_bomb_blast_tile(bomb_fire_right[bomb_idx], bomb_posy[bomb_idx]);		
	}
}

static void hit_actor_in_bomb_blast(UBYTE bomb_idx){
	actor_t *actor = actors_active_tail;
	
	const UBYTE a_x      = bomb_posx[bomb_idx];
    const UBYTE a_top    = bomb_fire_top[bomb_idx];
    const UBYTE a_bottom = bomb_fire_bottom[bomb_idx];
	const UBYTE a_y      = bomb_posy[bomb_idx];
    ...
    while (actor) {
		...
		if (((SUBPX_TO_TILE(actor->pos.x + actor->bounds.left) == a_x || SUBPX_TO_TILE(actor->pos.x + actor->bounds.right) == a_x) && 
		SUBPX_TO_TILE(actor->pos.y + actor->bounds.top) <= a_bottom && SUBPX_TO_TILE(actor->pos.y + actor->bounds.bottom) >= a_top) ||
		((SUBPX_TO_TILE(actor->pos.y + actor->bounds.bottom) == a_y || SUBPX_TO_TILE(actor->pos.y + actor->bounds.top) == a_y) && 
...
static void init_bomb_blast(UBYTE i){
	tile_col_test_range_y(COLLISION_BOTTOM, bomb_posx[i], bomb_posy[i] - 1, bomb_posy[i] - ((1 + bomb_range_upgrade) << 1));
	bomb_fire_top[i] = tile_hit_y & 0xFE;					
	tile_col_test_range_y(COLLISION_TOP, bomb_posx[i], bomb_posy[i] + 1, bomb_posy[i] + ((1 + bomb_range_upgrade) << 1));
	bomb_fire_bottom[i] = tile_hit_y & 0xFE;		
	tile_col_test_range_x(COLLISION_RIGHT, bomb_posy[i], bomb_posx[i] - 1, bomb_posx[i] - ((1 + bomb_range_upgrade) << 1));
	bomb_fire_left[i] = tile_hit_x & 0xFE;			
	tile_col_test_range_x(COLLISION_LEFT, bomb_posy[i], bomb_posx[i] + 1, bomb_posx[i] + ((1 + bomb_range_upgrade) << 1));
	bomb_fire_right[i] = tile_hit_x & 0xFE;		
	
	hit_actor_in_bomb_blast(i);
...
static void update_bombs(void) {
	//for (UBYTE i = 0; i < MAX_BOMBS; i++){
		...
		switch(bomb_state[i]){
			case 1://bomb init
			replace_meta_tile(bomb_posx[i], bomb_posy[i], 18, TRUE);
			bomb_state[i] = 2;
			bomb_timer[i] = 8;
			case 2://bomb update
			if (((game_time - i) & 15) == 0){
				replace_meta_tile(bomb_posx[i], bomb_posy[i], (16 + abs(2 - (bomb_timer[i] & 3))), TRUE);
				if (!bomb_timer[i]){
					bomb_state[i] = 3;
				} else {
					bomb_timer[i]--;
			...
			break;
			case 3://bomb init explosion		
			init_bomb_blast(i);	
			case 4://bomb update explosion
			update_bomb_blast(i);				
			if (!bomb_timer[i]){
				bomb_state[i] = 0;
				bomb_count--;
